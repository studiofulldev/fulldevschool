# 9 - Escolha Sua Trilha

**Objetivo:** transformar a área escolhida em uma sequência prática de estudo e execução.

**Passo anterior:** [[../07 - Comparando Caminhos/Comparando Caminhos|8 - Comparando Caminhos]]

**Próximo passo:** [[../09 - Projetos e Portfólio/Índice - Projetos e Portfólio|10 - Projetos e Portfólio]]

## Conteúdo

Agora você vai sair do campo da comparação e entrar no campo da execução.

Escolher uma trilha significa responder:

- o que estudar primeiro
- em que ordem estudar
- quais ferramentas realmente importam no começo
- que tipo de projeto faz sentido para você

## Como usar esta etapa

Você não precisa fazer várias trilhas ao mesmo tempo.

O melhor é:

1. escolher uma trilha principal
2. seguir por algumas semanas
3. produzir algo pequeno
4. reavaliar com base em prática real

- [[Frontend/Trilha Frontend|Trilha Frontend]]
- [[Backend/Trilha Backend|Trilha Backend]]
- [[Mobile/Trilha Mobile|Trilha Mobile]]
- [[QA/Trilha QA|Trilha QA]]
- [[UX UI Design/Trilha UX UI Design|Trilha UX UI Design]]
- [[Dados/Trilha Dados|Trilha Dados]]
- [[DevOps Cloud/Trilha DevOps Cloud|Trilha DevOps Cloud]]
- [[Cybersegurança/Trilha Cybersegurança|Trilha Cybersegurança]]
- [[Produto/Trilha Produto|Trilha Produto]]
- [[Suporte Infra/Trilha Suporte Infra|Trilha Suporte Infra]]

## Regra prática

Se você ainda está muito dividido, escolha a trilha que:

- parece mais estudável para você agora
- tem um primeiro projeto mais viável
- gera mais curiosidade do que resistência

## O que você deve sair sabendo ao terminar esta etapa

Ao concluir esta página, o ideal é que você tenha:

1. uma trilha principal escolhida
2. uma ordem inicial de estudo
3. uma noção clara do primeiro projeto possível

## Próxima ação prática

Siga para:

[[../09 - Projetos e Portfólio/Índice - Projetos e Portfólio|10 - Projetos e Portfólio]]

Essa é a etapa em que você transforma estudo em evidência prática.

## Verificação por tópico

| Tópico | Base usada | Taxa de assertividade | Observação |
| --- | --- | --- | --- |
| Como usar esta etapa | Diretriz editorial do projeto | 96% | Coerente com a arquitetura do guia. |
| Regra prática de escolha | Síntese editorial | 88% | Boa orientação, mas interpretativa. |
| Resultado esperado da etapa | Estrutura do próprio vault | 97% | Totalmente consistente com o projeto. |

## Bibliografia

### Fontes principais desta nota

1. PRD interno do projeto. `PRD — Guia de Entrada na Área de Tecnologia (Obsidian).md`
2. roadmap.sh. Disponível em: https://roadmap.sh/

### Mapeamento de fontes por assunto

- função desta etapa:
  `PRD — Guia de Entrada na Área de Tecnologia (Obsidian).md`
- noção de trilhas e progressão:
  https://roadmap.sh/

### Como interpretar a taxa de assertividade

- `95% a 100%`: informação diretamente sustentada por documentação ou dado forte.
- `85% a 94%`: síntese confiável, mas com algum grau de generalização editorial.
- `70% a 84%`: orientação útil, porém mais interpretativa e dependente de contexto.
